...
class Reviewer:
    ...
    def evaluate(self, structured_info: Dict[str, str], innovation_analysis: str, related_papers: list, timeout: Optional[int] = None) -> str:
        """
        多维度评估论文
        ...
        try:
            ...
            prompt = get_evaluation_prompt(info_text, innovation_analysis, related_text)
    ...
    def generate_review(self, structured_info: Dict[str, str], evaluation: str, innovation_analysis: str, timeout: Optional[int] = None) -> str:
        """
        生成评阅报告
        ...
        try:
            ...
            prompt = get_review_generation_prompt(info_text, evaluation, innovation_analysis)
            ...
            return self._generate_fallback_review(structured_info, evaluation, innovation_analysis)

    def _generate_fallback_review(self, structured_info: Dict[str, str], evaluation: str, innovation_analysis: str) -> str:
        """生成备用评阅报告（当LLM生成失败时）"""
...
*Note: This is a fallback review generated due to processing errors. Please review manually.*"""
    ...
    def review(self, structured_info: Dict[str, str], innovation_analysis: str, related_papers: list, timeout: Optional[int] = None) -> str:
        """
        完整的评阅流程
        ...
        try:
            # 1. 多维度评估
            evaluation = self.evaluate(structured_info, innovation_analysis, related_papers, timeout)
            
            # 2. 生成评阅报告
            review = self.generate_review(structured_info, evaluation, innovation_analysis, timeout)
            
            return review
        except Exception as e:
            print(f"⚠️  评阅流程失败: {e}")
            return self._generate_fallback_review(structured_info, "", innovation_analysis)
